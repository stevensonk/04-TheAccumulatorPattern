...
#
#              for k in range(r - s):           This does not include r or s
#                  count = 0                    count gets reset of zero every time
#                  if math.cos(r) > 0:          this uses the variable r instead of k, so there is no counting
#                      count = 1                count is set to 1 instead of adding 1
#                  return count                 count is returned within the loop, not after it
#
#     f. The code in the "graphics accumulation" example below includes:
#            for _ in range(n):
#        What does the   _   (underscore) mean?
#                   This is the counting variable, except it's not assigned a name. It isn't assigned
#                      a name because it isn't called in the code that follows.
#
